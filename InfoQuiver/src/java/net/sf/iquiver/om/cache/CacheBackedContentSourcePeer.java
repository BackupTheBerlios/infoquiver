    ...
    extends net.sf.iquiver.om.BaseContentSourcePeer
{
	...
	static
	{
		logger = LogFactory.getLog(CacheBackedContentSourcePeer.class);
		CACHE_KEY = "${targetDBOPackage}.torque.${databaseName}.om.ContentSource";
		useCache = false;
		clear_crit = new Criteria();

		if(CacheBackedPeer.isEnabledAtStartup())
		{
    		if(CacheBackedPeer.getExcludeEnableAtStartUp() != null && !ArrayUtil.containsMatchingRegExString(CacheBackedPeer.getExcludeEnableAtStartUp(), "de.dat.u3.db.dbo.torque.cd.om.ContentSourcePeer"))
    		{
				enableCache(true);
        	    if(CacheBackedPeer.isEnableCacheForCriteriaSelects())
        	    {
    				...
    				{
    					enableCacheForCriteriaSelects(!ArrayUtil.containsMatchingRegExString(CacheBackedPeer.getExcludeEnableCacheForCriteriaSelects(), "de.dat.u3.db.dbo.torque.cd.om.ContentSourcePeer"));
    				}
    				else
    				{
