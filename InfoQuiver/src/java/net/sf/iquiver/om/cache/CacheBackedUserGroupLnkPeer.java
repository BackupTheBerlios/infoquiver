    ...
    extends net.sf.iquiver.om.BaseUserGroupLnkPeer
{
	...
	static
	{
		logger = LogFactory.getLog(CacheBackedUserGroupLnkPeer.class);
		CACHE_KEY = "${targetDBOPackage}.torque.${databaseName}.om.UserGroupLnk";
		useCache = false;
		clear_crit = new Criteria();

		if(CacheBackedPeer.isEnabledAtStartup())
		{
    		if(CacheBackedPeer.getExcludeEnableAtStartUp() != null && !ArrayUtil.containsMatchingRegExString(CacheBackedPeer.getExcludeEnableAtStartUp(), "de.dat.u3.db.dbo.torque.cd.om.UserGroupLnkPeer"))
    		{
				enableCache(true);
        	    if(CacheBackedPeer.isEnableCacheForCriteriaSelects())
        	    {
    				...
    				{
    					enableCacheForCriteriaSelects(!ArrayUtil.containsMatchingRegExString(CacheBackedPeer.getExcludeEnableCacheForCriteriaSelects(), "de.dat.u3.db.dbo.torque.cd.om.UserGroupLnkPeer"));
    				}
    				else
    				{
